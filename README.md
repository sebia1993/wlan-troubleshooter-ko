...
실제 Windows 앱에 **저장소에서 생성한 합성 패킷**을 입력해 분석한 화면입니다. 현장 장애 해결 실적이나 정확도 보증으로 제시하지 않습니다.

![실제 분석 결과의 TCP RST 근거 Finding](docs/images/usage/03-findings.png)

[실행 준비 → 분석·취소 → 근거 Finding → 단말 여정 → PCAPNG 통계 → 잘못된 입력의 6단계 화면 안내](docs/USAGE_SCREENSHOTS_KO.md)에서 행동, 읽을 값과 다음 확인을 살펴볼 수 있습니다.
